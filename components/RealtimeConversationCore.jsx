...
export default function RealtimeConversationCore() {
  ...
  const [instantLatencyLabel, setInstantLatencyLabel] = useState("Instant local response ready");
  ...
  async function respond() {
    ...
    setStatus("Instant");
    setInstantLatencyLabel(`Instant local response: ${Math.max(0, Math.round(performance.now() - instantStartedAt))}ms`);
    setMessages((items) => [...items, { role: "You", text }, { role: "Genius", text: instant.response }]);
    setSpeaking(true);
  ...
  return (
    <section className="gg-realtime-core">
      ...
      <div className="gg-realtime-main">
        ...
        <p className="gg-note gg-latency-note">{instantLatencyLabel}</p>
