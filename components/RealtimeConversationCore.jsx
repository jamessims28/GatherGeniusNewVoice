...
import { runReadinessGate, readinessMessage } from "../lib/ambient-core/readinessGate";
...
export default function RealtimeConversationCore() {
  ...
  const [coreMode, setCoreMode] = useState("ambient operating core v1");
  ...
  async function unlockAndCheckReadiness() {
    ...
    const gate = await runReadinessGate({ selectedAudioDeviceId });
    setReadiness(gate);
    setReady(Boolean(gate.ready));

    const message = readinessMessage(gate);
  ...
  async function runCoreForVoice(text) {
    ...
    try {
      const response = await fetch("/api/ambient-core/run", {
        method: "POST",
      ...
      setCoreMode(core?.layer || "ambient operating core v1");
      ...
      const fallback = "I had trouble reaching the Ambient Operating Core. I can still listen once the connection is stable.";
  ...
  function startListening() {
    ...
    recognition.onresult = (event) => {
      ...
      setListening(false);
      setReadinessText("Processing through Ambient Operating Core V1.");
    ...
    <section className="gg-ambient-core-v1" onClick={unlockAndCheckReadiness} onTouchStart={unlockAndCheckReadiness}>
      ...
      <div className="gg-core-stage">
        ...
        <h1 className="gg-core-title">
          Ambient Operating Core
          <span>V1</span>
        </h1>

        <p className="gg-core-sub">
          Readiness gate, human intent, world signals, proactive decisions, outcome protection, and one premium voice interface.
        </p>
